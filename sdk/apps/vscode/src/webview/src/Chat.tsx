...
export default function Chat() {
	...
	return (
		<PromptInputProvider>
			<div className="relative flex size-full min-h-screen flex-col overflow-hidden">
							...
							if (message.role === "meta" || message.role === "error") {
								return (
									...
									>
										...
										{standardToolEvents.map((toolEvent) => (
											<Tool className="mt-3" defaultOpen key={toolEvent.id}>
												<ToolHeader
							...
							return (
								<Message from={message.role} key={message.id}>
									<div>
										...
										{standardToolEvents.map((toolEvent) => (
											<Tool className="mb-3" defaultOpen key={toolEvent.id}>
												<ToolHeader
													state={toolEvent.state}
													title={toolEvent.name}
													type="dynamic-tool"
